...
# Log-normal model: Does trial affect pupil size? {#sec:trial}
...
* assess the practical relevance of the effect for the research question; (only after 100 button presses do we see a slowdown of `r round(mean(effect_middle_ms),2) * 100` ms on average ($`r mean(effect_middle_ms)` \cdot 100$), with a 95% credible interval ranging from `r quantile(effect_middle_ms, .025)*100` to `r quantile(effect_middle_ms, .975)*100`);
